...
//    }
        ...
        if($result['msg_cd']=='000000') {
            ...
            if ($signValue == $result[ 'signValue' ]) {
                if ($result[ 'check_flag' ]==3) {
                    ...
                    TotalMerchant::where('id',$merchant_id)->update(['review_status'=>0,'opening_time'=>time()]);
                    if ($res) {
                        return_msg(200, '转人工');
                    } else {
                    ...
                    TotalMerchant::where('merchant_id',$merchant_id)->update(['review_status'=>2,'opening_time'=>time()]);
                    if ($res) {
                        return_msg(200, '进件成功');
                    } else {
                        return_msg(400, 'error');
                    ...
                    //驳回
                    TotalMerchant::where('merchant_id',$merchant_id)->update(['review_status'=>3,'rejected'=>$result[ 'msg_dat'],'opening_time'=>time()]);
                    return_msg(400 , $result[ 'msg_dat' ]);
                }
            }
        }else{
    ...
    public function img_upload(Request $request)
    {
        ...
        $info=$request->post();
        //取出当前商户信息
        $data=MerchantIncom::where('merchant_id',$info['merchant_id'])->field('mercId,log_no,stoe_id')->find();
